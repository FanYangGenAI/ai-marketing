...
输出格式：
- 封面图视觉方向（构图、主体、配色、情绪）
- 正文配图列表（每张图的画面内容描述，中文 + English prompt + 尺寸）
- 整体视觉基调（风格词 3-5 个）
注意：图片描述要足够具体，可以直接用于调用 AI 图像生成工具。

⚠️ 配图尺寸选择规则（必须为每张图明确指定）：
- 3:4（竖版，默认）：单一主体、产品特写、人物照、封面图
- 16:9（横版）：两个或以上对象横向并排的构图（如"操作步骤对比"、"功能前后对比"、"多手机并列展示"）
- 1:1（方形）：产品 UI 局部截图、图标类
关键原则：凡画面中有两个或多个对象横向排列，必须使用 16:9，否则裁剪后两侧内容丢失。"""
...
⚠️ 尺寸选择规则（必须严格执行，不可全部填 3:4）：
- **3:4**：单一主体、封面图、人物照、产品特写
- **16:9**：两个或以上对象横向并排（步骤对比图、功能对比图、多界面并列）→ 此类图若用 3:4 裁剪后两侧内容必然丢失
- **1:1**：产品 UI 局部截图、图标
