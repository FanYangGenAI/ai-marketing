...
def next_attempt_id(attempts_root: Path) -> str:
    """
    Next sequential attempt folder name under attempts_root: attempt_00, attempt_01, ...
    Creates attempts_root if missing.
    """
    ...
    for p in attempts_root.iterdir():
        if not p.is_dir():
            continue
        m = re.fullmatch(r"attempt_(\d+)", p.name)
        if m:
            max_n = max(max_n, int(m.group(1)))
    return f"attempt_{max_n + 1:02d}"
